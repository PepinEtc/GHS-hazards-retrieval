...
def draw_mainframe():
    ...
    def get_compound(name, name_lbl, class_lbl, CMR_lbl, GHS_lbl, img_lbl):
        global compound_name, compound_class
        try:
    ...
    search_frame = Frame(bottom_frame)
   ...
   # img_lbl.image = imgTK
    ...
    btn = Button(search_frame, text="Search", command=lambda: get_compound(search_box.get(), name_lbl, class_num_lbl, cmr_lbl, GHS_lbl, img_lbl))
    ...
    excel_btn = Button(search_frame, text="Copy to excel", command=lambda: addToClipBoard(f'{search_box.get()}\t{class_num_lbl.cget('text')}\t{cmr_lbl.cget('text')}'))
    excel_btn.pack()
